...
void writeStubs(FILE *fd, List *list) {
    int i;
    ...
    for(i = 0; i < list->count; i++) {
        ...
        char *mangled = mangleSig(sig, list == &static_sigs);
...
void writeStubsTable(FILE *fd, List *list) {
    int i;
...
void writeStubsFile(char *stubs_name, char *sigs_name) {
    ...
    fprintf(fd, "#include \"jni-stubs.h\"\n");
    fprintf(fd, "\nextern void *jni_env;\n\n");

    fprintf(fd, "/* Static signatures */\n\n");
    writeStubs(fd, &static_sigs);

    fprintf(fd, "/* Non-static signatures */\n\n");
    writeStubs(fd, &non_static_sigs);

    writeStubsTable(fd, &static_sigs);
    writeStubsTable(fd, &non_static_sigs);
